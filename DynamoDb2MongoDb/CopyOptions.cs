namespace DynamoDb2MongoDb
{
    ...
    public class CopyOptions
    {
        ...
        public static IEnumerable<Example> Examples
        {
            get
            {
                return new List<Example>() {
                    new Example("Copy data from source DynamoDb table to target MongoDb collection",
                    new CopyOptions()
                    {
                        DynamoAccessKey = "***",
        ...
        /// <summary>
        /// Gets or sets the DynamoAccessKey.
        /// </summary>
        [Option("dynamoaccesskey", HelpText = "Source AWS access key with DynamoDb access.", Required = true)]
        public string DynamoAccessKey { get; set; }

        /// <summary>
        /// Gets or sets the DynamoSecretAccesKey.
        /// </summary>
        [Option("dynamosecretaccesskey", HelpText = "Source AWS secret access key with DynamoDb access.", Required = true)]
        public string DynamoSecretAccesKey { get; set; }

        /// <summary>
        /// Gets or sets the DynamoRegion.
        /// </summary>
        [Option("dynamoregion", HelpText = "Source AWS region with DynamoDb service.", Required = true)]
        public string DynamoRegion { get; set; }

        /// <summary>
        /// Gets or sets the DynamoTable.
        /// </summary>
        [Option("dynamotable", HelpText = "Source DynamoDb table.", Required = true)]
        public string DynamoTable { get; set; }

        /// <summary>
        /// Gets or sets the MongoConnectionString.
        /// </summary>
        [Option("mongoconnectionstring", HelpText = "Target MongoDb connection string.", Required = true)]
        public string MongoConnectionString { get; set; }

        /// <summary>
        /// Gets or sets the MongoCollection.
        /// </summary>
        [Option("mongocollection", HelpText = "Target MongoDb collection. Uses source DynamoDb table as default.")]
        ...
        public static int Copy(CopyOptions opts)
        {
            ...
            {
                // Load connections
                var dynamodbContext = new AmazonDynamoDBClient(opts.DynamoAccessKey, opts.DynamoSecretAccesKey, RegionEndpoint.GetBySystemName(opts.DynamoRegion));
                var mongodbContext = new MongoDbContext(opts.MongoConnectionString);
                if (string.IsNullOrEmpty(opts.MongoCollection)) opts.MongoCollection = opts.DynamoTable;
        ...
        private static string GetDynamoDocument(Document item)
        {
            ...
            foreach (var value in item)
            {
                ...
                // String array
                if (value.Value is PrimitiveList && ((PrimitiveList)value.Value).Type == DynamoDBEntryType.String)
                {
                    writer.WritePropertyName(value.Key);
                    writer.WriteStartArray();
                    foreach (var v in ((PrimitiveList)value.Value).AsListOfString()) writer.WriteStringValue(v);
                    writer.WriteEndArray();
                    continue;
                }

                // String & Nested Json
                if (value.Value is Primitive && ((Primitive)value.Value).Type == DynamoDBEntryType.String)
                {
                    ...
                    {
                        writer.WriteString(value.Key, value.Value); continue;
                    }
                }
            ...
            foreach (var placeholder in placeholders) result = result.Replace($"\"{placeholder.Key}\"", placeholder.Value);
